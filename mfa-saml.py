...
idpentryurl = 'https://fs.effctech.co.uk/adfs/ls/IdpInitiatedSignOn.aspx?loginToRp=urn:amazon:webservices'
...
username='pcridland@effctech.org'
password='Tescobank123'
...
while 'VerificationCode' not in payload3:
    mfasoup = BeautifulSoup(mfaresponse.text.decode('utf8'), "lxml")
    ...
    print 'trying url {} with payload: {}'.format(mfaurl, payload2)
    mfaresponse = session.post(
            mfaurl, data=payload2, verify=sslverification)
    sleep(1)
